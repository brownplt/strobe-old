...
(define-language λtss
  ; Types
  (T (T -> T) (T U T) int str bool)
  ; Expressions
  (e v x (equal? e e) (typeof e) (let (x e) e) (if e e e) (e e))
  ; Values
  (v number #t #f string (lambda (x : T) e) + (number +) ++ (string ++))
  ; Identifiers
...
(define-metafunction λtss
  ...
  [(subst x_1 v_1 (lambda (x_1 : T_1) e_1))
   (lambda (x_1 : T_1) e_1)]
  [(subst x_1 v_1 (lambda (x_2 : T_1) e_1)) ; capture avoiding substitution
   ,(term-let ([x_21 (variable-not-in (term (v_1 e_1)) (term x_2))])
              (term (lambda (x_21 : T_1) (subst x_1 v_1 (subst-var x_2 x_21 e_1)))))]
...
(define red
  (reduction-relation 
   ...
   (--> (in-hole E (typeof #f))
        (in-hole E "boolean"))
   (--> (in-hole E (typeof (lambda (x_1 : T_1) e_1)))
        (in-hole E "function"))
   ...
   (--> (in-hole E ((lambda (x_1 : T_1) e_1) v_1))
        (in-hole E (subst x_1 v_1 e_1)))))



(define generic-add
  (term (lambda (x : (int U (str U bool)))
...
;(current-traced-metafunctions '(subst))
(define e1
