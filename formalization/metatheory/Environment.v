...
Lemma binds_in_tail : forall x y (a:A) (b:A) E,
  x <> y -> binds x a E -> binds x a ((y,b) :: E).
Proof.
  intros x y a b E.
  intros neq. unfold not in neq.
  unfold binds.
  simpl.
  destruct (eq_atom_dec x y). apply neq in e. inversion e.
  intros H. exact H.
Qed.

Lemma binds_at_head : forall x (a:A) E,
  binds x a ((x,a) :: E).
Proof.
  intros x a E.
  unfold binds. simpl.
  destruct (eq_atom_dec x x). reflexivity. 
